...
/**
 * Graph state: messages + optional subagentId and model options (persisted for resume reuse).
 */
export const AgentState = Annotation.Root({
  messages: Annotation<BaseMessage[]>({ reducer: messagesStateReducer, default: () => [] }),
  __interrupt__: Annotation<Interrupt[]>(),
  /** Subagent name from run/resume; when set, that subagent's runnable is used. Persisted so resume can omit. */
  subagentId: Annotation<string | undefined>({ reducer, default: () => undefined }),
